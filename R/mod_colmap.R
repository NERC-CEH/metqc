...
mod_colmap_server <- function(id, uploaded, dictionary_list) {
  moduleServer(id, function(input, output, session) {
    ...
    output$dict_set_ui <- renderUI({
      req(uploaded())
      normal_dicts <- setdiff(
        names(dictionary_list),
        c("long_name", "all_units")
      )
      selectInput(
        ns("dict_set"),
        "Select dictionary set",
          ...
          lapply(cols, function(col) {
            fluidRow(
              ...
              column(
                4,
                selectInput(
                  ns(paste0("map_long_", col)),
                  NULL,
                  choices = c(col, long_names),
                  selected = col
                )
              ),
              column(
      ...
      if (input$dict_set == "Custom") {
        ...
        new_names <- sapply(
          ...
          function(col) {
            ...
            base_name <- if (!is.null(input[[paste0("map_", col)]])) {
            ...
            if (add_sensor) {
              ...
              boxes <- c(b1, b2, b3)
              boxes <- boxes[
                !sapply(boxes, function(x) is.null(x) || is.na(x) || x == "")
              ]
              if (length(boxes) > 0) {
        ...
        new_names <- sapply(
          names(df),
          function(col) {
            if (!is.null(input[[paste0("map_", col)]])) {
              input[[paste0("map_", col)]]
            } else {
              col
            }
    ...
    observeEvent(input$reset_mapping, {
      ...
      if (!is.null(input$dict_set) && input$dict_set == "Custom") {
        ...
        lapply(cols, function(col) {
          ...
          try(
            updateSelectInput(session, paste0("map_", col), selected = col),
            silent = TRUE
          )
          # update era5 boxes if present
          try(
            updateNumericInput(session, paste0("era5_box1_", col), value = 1),
            silent = TRUE
          )
          try(
            updateNumericInput(session, paste0("era5_box2_", col), value = 1),
            silent = TRUE
          )
          try(
            updateNumericInput(session, paste0("era5_box3_", col), value = 1),
            silent = TRUE
          )
        })
      }
