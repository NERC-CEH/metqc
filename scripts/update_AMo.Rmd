...
# level 1 data will be more up-to date so longer, non-congruent time series
# subset level 1 data to range of era5 data
l_lev1$df    <- l_lev1$df[   DATECT >= min(df_era5$DATECT) & DATECT <= max(df_era5$DATECT)]
l_lev1$df_qc <- l_lev1$df_qc[DATECT >= min(df_era5$DATECT) & DATECT <= max(df_era5$DATECT)]

# reciprocally subset era5 data to range of level 1 data
# era5 data contains one extra value at 1995-01-01 00:00 (not 00:30) so longer, non-congruent time series
df_era5    <- df_era5[DATECT >= min(l_lev1$df$DATECT) & DATECT <= max(l_lev1$df$DATECT)]
